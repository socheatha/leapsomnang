	...
	<div class="card-header">
		...
		<div class="card-tools">
			...
			@component('components.action')
				@slot('otherBTN')
					@can('Usage Create')
					<a href="{{route('usage.create')}}" class="dropdown-item"><i class="fa fa-plus"></i> &nbsp;{{ __('label.buttons.create') }}</a>
					@endcan
				@endslot
			@endcomponent
		...
		<table id="datatables-2" class="table table-striped table-hover">
			<thead>
				<tr>
					...
					<th width="10%">{!! __('module.table.usage.code') !!}</th>
				...
				@foreach($usages as $i => $usage)
					<tr>
						...
						<td class="text-right">
							...
							@can('Usage Delete')
							{{-- Delete Button --}}
							<button class="btn btn-danger btn-sm btn-flat BtnDeleteConfirm" value="{{ $usage->id }}" data-toggle="tooltip" data-placement="left" title="{{ __('label.buttons.delete') }}"><i class="fa fa-trash-alt"></i></button>
