		...
		function getDatatable(from, to) {
			...
			dataTableInvoice = $('#invoice_table').DataTable({
				...
				rowCallback: function( row, data ) {
					...
					$('td:eq(9)', row).html( `@Can("Invoice Edit")
					...
					$('td:eq(5)', row).html( `@Can("Invoice Status")<span class="badge badge-${ ((data.status==1)? 'success' : '') }">${ ((data.status==1)? 'paid' : 'upaid') }</span>@endCan` );
