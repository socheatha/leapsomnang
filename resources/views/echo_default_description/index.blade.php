	...
	<div class="card-header">
		...
		<div class="card-tools">
			...
			@component('components.action')
				@slot('otherBTN')
					@can('Echo Default Description Create')
					<a href="{{route('echo_default_description.create')}}" class="dropdown-item"><i class="fa fa-plus"></i> &nbsp;{{ __('label.buttons.create') }}</a>
					@endcan
				@endslot
			@endcomponent
		...
		<table id="datatables-2" class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th width="10%">{!! __('module.table.echo_default_description.slug') !!}</th>
			...
			<tbody>
				@foreach($echo_default_descriptions as $i => $echo_default_description)
					<tr>
						...
						<td>{{ $echo_default_description->name }}</td>
						<td>{{ $echo_default_description->slug }}</td>
						<td>{!! $echo_default_description->description !!}</td>
						<td class="text-right">

							@can('Echo Default Description Edit')
							{{-- Edit Button --}}
							<a href="{{ route('echo_default_description.edit', $echo_default_description->id) }}" class="btn btn-info btn-sm btn-flat" data-toggle="tooltip" data-placement="left" title="{{ __('label.buttons.edit') }}"><i class="fa fa-pencil-alt"></i></a>
							@endcan

							@can('Echo Default Description Delete')
							{{-- Delete Button --}}
							<button class="btn btn-danger btn-sm btn-flat BtnDeleteConfirm" value="{{ $echo_default_description->id }}" data-toggle="tooltip" data-placement="left" title="{{ __('label.buttons.delete') }}"><i class="fa fa-trash-alt"></i></button>
